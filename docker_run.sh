...
###运行app0-tomcat
echo 运行app0-tomcat
docker run --name=app0-tomcat --link=app0-master:master --link=app0-slave:slave -v `pwd`/conf/app0/tomcat-ibcp/ibcp:/srv/ibcp -d `docker images -q ibcp-all`
###运行app0-nginx
echo 运行app0-nginx
docker run --name=app0-nginx --link=app0-tomcat:tomcat -v `pwd`/conf/app0/nginx-ibcp/conf.d:/etc/nginx/conf.d -d `docker images -q avatech/ibcp-nginx`
#运行app1
###运行app1-master
echo 运行app1-master
...
###运行app1-tomcat
echo 运行app1-tomcat
docker run --name=app1-tomcat --link=app1-master:master --link=app1-slave:slave -v `pwd`/conf/app1/tomcat-ibcp/ibcp:/srv/ibcp -d `docker images -q ibcp-all`
###运行app1-nginx
echo 运行app1-nginx
docker run --name=app1-nginx --link=app1-tomcat:tomcat -v `pwd`/conf/app1/nginx-ibcp/conf.d:/etc/nginx/conf.d -d `docker images -q avatech/ibcp-nginx`
#运行nginx-porxy
echo 运行nginx-porxy
